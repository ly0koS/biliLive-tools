    ...
    <draggable v-model="fileList" :options="options" item-key="id" handle=".handle">
      <template #item="{ element, index }">
        <div :key="element.id" class="draggable">
          ...
          <span
            v-if="element.subFilePath"
            :title="element.subFilePath"
            style="display: inline-flex"
            class="apd-icon"
            @click="removeSubFile(index)"
          >
            <slot name="subFileOn">
              <svg
                class="apd-toggle-on"
          ...
          <span
            ...
            style="display: inline-flex"
            class="apd-icon"
            @click="addSubFile(index)"
          >
            <slot name="subFileOff">
              <svg
                class="apd-toggle-off"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                xmlns="http://www.w3.org/2000/svg"
                data-pointer="none"
                viewBox="0 0 24 24"
...
const emits = defineEmits<{
  (event: "addSubFile", value: number): void;
}>();
...
const addSubFile = (index: number) => {
  emits("addSubFile", index);
};
const removeSubFile = (index: number) => {
  fileList.value[index].subFilePath = "";
};
</script>
